...
## Usage
If you just want to run the script with the default preset, download and unpack the [latest release](https://github.com/petrak-dan/Win11-Initial-Setup-Script) and then simply double-click on the *Default.cmd* file and confirm *User Account Control* and *Terms of Use* prompts. Make sure your account is a member of *Administrators* group as the script attempts to run with elevated privileges.

The script supports command line options and parameters which can help you customize the tweak selection or even add your own custom tweaks and skip the *Terms of Use* prompt for using the script in automated manner, however these features require some basic knowledge of command line usage and PowerShell scripting. Refer to [Advanced usage](#advanced-usage) section for more details.
...
**A:** The script aims to be compatible with the most up-to-date 64bit version of Windows 10 / Windows 11 receiving updates from semi-annual channel, however if you create your own preset and exclude the incompatible tweaks, it will work also on LTSB/LTSC, Server editions and possibly also on 32bit systems. Vast majority of the tweaks will work on all Windows editions. Some of them rely on group policy settings, so there may be a few limitations for Home and Education editions.

**Q:** Can I run the script on Windows Server 2016 or 2019?  
**A:** Yes. Starting from version 2.5, Windows Server is supported. There are even few tweaks specific to Server environment. Keep in mind though, that the script is still primarily designed for Windows 10 / Windows 11, so you have to create your own preset.

**Q:** Can I run the script on Windows 7, 8, 8.1 or other versions of Windows?  
**A:** No. Although some tweaks may work also on older versions of Windows, the script is developed only for Windows 10 and Windows 11. There are no plans to support older versions.
...
**A:** The author of the original projects cared about possibility to directly taking a function block or a line from within a function and using it elsewhere, without elaborating on any dependencies. Some new tweaks however, may have the opposite approach and if they use a custom function, it should be found in the Auxiliary Functions region of the script module.
...
I recommend you to visit the [List of Microsoft Windows versions](https://en.wikipedia.org/wiki/List_of_Microsoft_Windows_versions) web page on Wikipedia.
